...
> **IMPORTANT:** This project uses npm. Run all commands with `npx shadcn@latest`.
...
1. **Named:** `--preset base-nova` or `--preset base-mira` (this project uses `base-mira`)
2. **Code:** `--preset a2r6bw` (base62 string, starts with lowercase `a`)
3. **URL:** `--preset "https://ui.shadcn.com/init?base=base&style=mira&..."`
...
Always run preset commands inside the user's project directory. The CLI automatically preserves the current base from `components.json`. If you must use a scratch/temp directory (e.g. for `--dry-run` comparisons), pass `--base base` explicitly — preset codes do not encode the base.
